...
class LaminateCalculatorService {
  LaminateCalculation calculate({
    ...
    if (roomLength <= 0) {
      throw ArgumentError.value(roomLength, 'roomLength', 'Длина комнаты должна быть больше 0');
    }
    if (roomWidth <= 0) {
      throw ArgumentError.value(roomWidth, 'roomWidth', 'Ширина комнаты должна быть больше 0');
    }
    if (panelLength <= 0) {
      throw ArgumentError.value(panelLength, 'panelLength', 'Длина панели должна быть больше 0');
    }
    if (panelWidth <= 0) {
      throw ArgumentError.value(panelWidth, 'panelWidth', 'Ширина панели должна быть больше 0');
    }
    if (panelsPerPack <= 0) {
      throw ArgumentError.value(panelsPerPack, 'panelsPerPack', 'Панелей в упаковке должно быть больше 0');
    }
    if (wastePercent < 0) {
    ...
    if (panelArea <= 0) {
      throw ArgumentError.value(panelArea, 'panelArea', 'Площадь панели должна быть больше 0');
    }

    final panelsWithoutWaste = roomArea / panelArea;
    final panelsWithoutWasteRounded = panelsWithoutWaste.ceil();
    final panelsWithWaste = (panelsWithoutWaste * (1 + wastePercent / 100)).ceil();
